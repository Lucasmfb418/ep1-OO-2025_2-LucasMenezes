...
public class GerenciadorDeArquivos {
    ...
    public static ArrayList<Paciente> carregarPacientes(String cam) {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(cam))) {
            ...
            while((linha = br.readLine()) != null) {
                ...
                if(tipo.equals("Especial")) {
                    lista.add(new PacienteEspecial(nome, cpf, idade, null));
                } else {
    ...
    public static void salvarPacientes(String cam, ArrayList<Paciente> pacientes) {
        try (PrintWriter pw = new PrintWriter(new FileWriter(cam))) {
            for(Paciente p : pacientes) {
                ...
                pw.println(tipo + ";" + p.getNome() + ";" + p.getCpf() + ";" + p.getIdade());
            }
        } catch (IOException e) {
    ...
    public static void salvarPlanos(String cam, ArrayList<PlanoSaude> planos) {
        try (PrintWriter pw = new PrintWriter(new FileWriter(cam))) {
            for(PlanoSaude p : planos) {
                ...
                sb.append(p.getNome());
                for(Map.Entry<Object, Object> entry : p.getDescontos().entrySet()) {
    ...
    public static ArrayList<Consulta> carregarConsultas(String cam, ArrayList<Paciente> pacientes, ArrayList<Medico> medicos) {
        ...
        try(BufferedReader br = new BufferedReader(new FileReader(cam))){
            ...
            while((linha = br.readLine()) != null) {
                ...
                LocalDateTime dt = LocalDateTime.parse(dados[2], DateTimeFormatter.ofPattern("dd-MM-yyyyy HH:mm"));
    ...
    public static void salvarConsultas(String cam, ArrayList<Consulta> consultas){
        try(PrintWriter pw = new PrintWriter(new FileWriter(cam))){
            for(Consulta c : consultas){
                pw.println(c.getPaciente().getCpf() + ";" + c.getMedico().getCrm() + ";" + c.getDataHora().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm")) + ";" + c.getLocal() + ";" + c.getStatus() + ";" + c.getDiagnostico() + ";" + c.getPrescricao());
            }
        } catch(IOException e){
    ...
    public static ArrayList<Internacao> carregarInternacoes(String cam, ArrayList<Paciente> pacientes, ArrayList<Medico> medicos){
        ...
        try(BufferedReader br = new BufferedReader(new FileReader(cam))){
            ...
            while((linha = br.readLine())!=null){
                ...
                String cpfPaciente = dados[0];
                String crmMedico = dados[1];
                LocalDateTime entrada = LocalDateTime.parse(dados[2], DateTimeFormatter.ofPattern("dd-MM-yyyy HH:mm"));
    ...
    public static void salvarInternacoes(String cam, ArrayList<Internacao> internacoes){
        try(PrintWriter pw = new PrintWriter(new FileWriter(cam))){
            for(Internacao i : internacoes){
                pw.println(i.getPaciente().getCpf() + ";" + i.getMedicoResponsavel().getCrm() + ";" + i.getDataEntrada().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm")) + ";" + (i.getDataSaida()==null ? "null" : i.getDataSaida().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm"))) + ";" + i.getQuarto() + ";" + i.getCusto() + ";" + i.getStatus());
            }
        } catch(IOException e){
